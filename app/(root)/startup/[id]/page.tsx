...
const page = async ({ params }: { params: Promise<{ id: string }> }) => {
    ...
    // this was sequential fetching
    // const post = await client.fetch(STARTUP_BY_ID_QUERY, {
    //     id,
    // });

    // // editor picks is the slug created in sanity referring to a playlist schema slug
    // const { select: editorPosts } = await client.fetch(PLAYLIST_BY_SLUG_QUERY, { slug: "editor-picks" });

    // instead of sequential, let's do parallel fetching to decrease time of querying. they are independent queries so we can run them in parallel
